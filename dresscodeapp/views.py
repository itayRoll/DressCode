...
def get_questions_feed(request):
	...
	answered_ids = [a.question_id for a in Answer.objects.filter(user__username=curr_username)]
	questions_feed = Question.objects.filter(due_date__lt=timezone.now()).exclude(user__username=curr_username, pk__in=answered_ids).order_by('-published_date')
